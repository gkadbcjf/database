...
JOIN DEPARTMENT ON (DEPT_CODE =DEPT_ID)
JOIN LOCATION ON (LOCATION_ID =LOCAL_CODE)
JOIN NATIONAL USING (NATIONAL_CODE); --저장할거다
...
--200번 사원의 급여를 200만원으로 변경(SALARY >=3000000조건에 맞지않는 변경)
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID =200; --조건에 맞지 않아 200번은 삭제
...
AS SELECT *
    FROM EMPLOYEE
    WHERE SALARY >=3000000
WITH CHECK OPTION; --변경을 막아줌

--200번 사원의 급여를 200만원으로 변경(SALARY >=3000000조건에 맞지않는 변경)이 되지 않는다.
UPDATE VW_EMP
SET SALARY = 2000000
WHERE EMP_ID =200; --아까와 달리 행을 삭제하지 않고 에러를 발생시킴
