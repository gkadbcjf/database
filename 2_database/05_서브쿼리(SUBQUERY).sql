...
--간단한 서브쿼리 예시2
--전 직원의 평균급여보더 더 많은 급여를 받는 사원들의 사번, 이름, 직급코드, 급여 조회
--1) 전직원의 평균급여
...
--2) 최저급여를 받는 사원의 사번, 이름, 급여, 입사실 조회
...
WHERE DEPT_CODE =DEPT_ID AND DEPT_CODE =(SELECT DEPT_CODE
                    FROM EMPLOYEE
                    WHERE EMP_NAME ='전지연') AND NOT EMP_NAME='전지연';
--단, 전지연사원은 제외;
...
/*
    ...
    IN (서브쿼리) : 여러개의 결과 값 중에서 한개라도 일치하는 값이 있다면 조회
    ...
    < ALL (서브쿼리) : 여러개의 모든 결과값들 보다 작을 겨우
        비교대상 > ALL (서브쿼리의 결과값 => 값1, 값2, 값3...)
        비교대상 > 값1 AND 비교대상 > 값2 AND 비교대상 > 값3...
*/

--1) 유재식 또는 윤은해사원의 같은 직급인 사원들의 사번, 사원명, 직급코드, 급여 조회

--1-1) 유재식 또는 윤은해사원의 직급코드
SELECT JOB_CODE
FROM EMPLOYEE
WHERE EMP_NAME IN ('유재식','윤은해');

--2-2) 직급이 J3,J7인 사원들의 조회
SELECT EMP_ID,EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
--2_2) 대리직급이면서 위의 결과 값 보다 하나라도 큰 사원
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME ='과장') AND JOB_NAME ='대리';
...
SELECT  DEPT_CODE,급여
FROM (SELECT ROUND(AVG(SALARY)) AS "급여",DEPT_CODE
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
...
--(부서코드, 부서별 총 급여합, 부서별 평균급여, 부서별 사원수) 

SELECT *
FROM(SELECT DEPT_CODE, SUM(SALARY) AS "총합", ROUND(AVG(SALARY)) AS "평균급여", COUNT(*) AS "사원수"
    FROM EMPLOYEE 
    GROUP BY DEPT_CODE)
WHERE 평균급여>2700000;
...
SELECT *
FROM(SELECT DEPT_CODE, SUM(SALARY) AS "총합", ROUND(AVG(SALARY)) AS "평균급여", COUNT(DEPT_CODE) AS "사원수"
    FROM EMPLOYEE 
...
FROM EMPLOYEE; --출력하면 공동 19등 2명, 그 뒤의 등수는 21등으로 하나 건너 뛴 걸 알 수 있다.
