...
--DEPARTMENT테이블에 모든 컬럼 조회
SELECT * FROM DEPARTMENT;
--DEPARTMENT테이블에 부서코드, 부서명 조회
SELECT DEPT_ID,DEPT_TITLE FROM DEPARTMENT;
--EMPLOYEE테이블에 사원명, 이메일, 전화번호 입사일,급여 조회
...
-- EMPLOYEE테이블의 사원명, 급여, 보너스, 연봉, 보너스포함연봉 조회(급여 * 보너스) *12
...
SELECT EMP_NAME 사원명, SALARY AS 고구마빵, BONUS "죽빵" , (SALARY * 12) as "연봉(원)" , 
...
/*
    <DISTINCT>
    중복제거 - 컬럼에 표시된 값들을 한번씩만 조회하고할 때 사용
*/
...
--위처럼 작성하면 에러가 발생한다. DISTINCT는 한 번만 사용이 가능하다.
...
-- EMPLOYEE에서 부서코드가 'D1'인 사원들의 사원명, 급여, 부서코드를 조회
SELECT EMP_NAME,SALARY,DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE ='D9';

-- EMPLOYEE에서 부서코드가 'D1'이 아닌 사원들의 사원명, 급여, 부서코드를 조회
SELECT EMP_NAME,SALARY,DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE !='D9';
...
SELECT * FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN '90/01/01' AND '01/01/01';--DATE타입도 비교연산 가능
...
/*
    ...
    '%' : 포함문자 검색 (0글자이상 전부 조회)
    EX) 비교할 대상 컬럼 LIKE '문자%' : 비교대상 컬럼 값 중에서 해당문자로 시작하는 값들만 조회
        비교할 대상 컬럼 LIKE '%문자' : 비교대상 컬럼 값 중에서 해당문자로 끝나는 값들만 조회
        비교할 대상 컬럼 LIKE '%문자%' : 비교대상 컬럼 값 중에서 해당문자가 포함된 값 조회    
        
    '_' : 1글자를 대체하는 검색
    EX) 비교할 대상 컬럼 LIKE '_문자' : 비교대상 컬럼 값 문자앞에 아무글자나 N글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '문자_' : 비교대상 컬럼 값 문자뒤에 아무글자나 N글자가 있는 값을 조회
        비교할 대상 컬럼 LIKE '_문자_' : 비교대상 컬럼 값 문자앞,뒤에 아무글자나 N글자가 있는 값을 조회
*/
...
--사원들 중에서 이름에 중간글자가 '하'인 사원의 이름 전화번호 목록 조회
SELECT EMP_NAME,PHONE
FROM EMPLOYEE
...
FROM EMPLOYEE
--WHERE EMAIL LIKE '____%'; 와일드카드문자때문에 정상출력이 되지않는다.
...
--직급코드가 J7 이거나 J2인 사원들 중에 급여가 200만원 이상인 사원들의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
--5. 보너스를 포함 연봉이 NULL이 아니고 이름에 '하'가 포함된 사원들의 사번, 사원명, 급여, 보너스 포함 연봉을 구하시오.
