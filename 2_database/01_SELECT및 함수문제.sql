...
SELECT *
FROM EMPLOYEE
WHERE ROUND(TO_CHAR(SYSDATE-HIRE_DATE)/365) >=20;
...
,CEIL((SYSDATE-TO_DATE(SUBSTR(EMP_NO,1,6)))/365) AS 나이
FROM EMPLOYEE;
-- ||이용가능
...
-- 주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6),SUBSTR(EMP_NO,8),TO_NUMBER(SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8))
FROM EMPLOYEE
WHERE EMP_ID ='201';
...
--SELECT COUNT(*), COUNT(SUBSTR(HIRE_DATE,1,2))
--FROM EMPLOYEE
...
SELECT COUNT(DECODE(TO_CHAR(HIRE_DATE,'YYYY'),'2001',1)) AS "2001년",
