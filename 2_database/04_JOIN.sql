/*
    ...
    ----------------------------------------------------------------------------           
                포괄조인                   |        왼족 외부 조인(LEFT OUTER JOIN)
              (LEFT OUTER)               |       오른쪽 외부 조인(RIGTH OUTER JOIN)
              (RIGTH OUTER)              |         전체 외부 조인(FULL OUTER JOIN)
   -----------------------------------------------------------------------------
        자체조인(SELF JOIM)                |                   JOIN ON
...
--2. DEPARTMENT와 LOCATION테이블을 참고하여 전체 부서의 
...
SELECT EMP_NAME, DEPT_TITLE,SALARY,SALARY * 12
FROM EMPLOYEE
RIGHT OUTER JOIN DEPARTMENT ON( DEPT_CODE =DEPT_ID); --LEFT,RIGHT OUTER(생략가능)
...
--오라클
SELECT E.EMP_ID, E.EMP_NAME,J.JOB_NAME DEPT_TITLE, L.LOCAL_NAME,N.NATIONAL_NAME,SAL_LEVEL
FROM EMPLOYEE E, DEPARTMENT,LOCATION L, NATIONAL N, SAL_GRADE, JOB J
WHERE DEPT_CODE =DEPT_ID AND LOCATION_ID =L.LOCAL_CODE AND N.NATIONAL_CODE = L.NATIONAL_CODE AND E.JOB_CODE =J.JOB_CODE
AND SALARY BETWEEN MIN_SAL AND MAX_SAL;

--ANSI
SELECT EMP_ID, EMP_NAME,JOB_NAME, DEPT_TITLE, LOCAL_CODE,NATIONAL_NAME,SAL_LEVEL
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE =DEPT_ID)
JOIN LOCATION ON(LOCATION_ID =LOCAL_CODE)
JOIN NATIONAL USING (NATIONAL_CODE)
