...
SELECT SUM(SALARY) FROM EMPLOYEE; --전체 사원을 하나의 그룹으로 묶어서 총 합을 구한 결과

-- 각 부서별 총 급여

SELECT NVL(DEPT_CODE,'NONE') ,SUM(SALARY)AS "부서별 총 급여" FROM EMPLOYEE 
GROUP BY DEPT_CODE ;
...
SELECT DEPT_CODE ,COUNT(*),SUM(SALARY) FROM EMPLOYEE 
...
SELECT JOB_CODE AS "부서 코드", COUNT(*)AS "직급수",COUNT(BONUS) AS "보너스 수",
SUM(SALARY)AS "급여 합",ROUND(AVG(SALARY)) AS "평균급여",
MIN(SALARY)AS "최소급여",MAX(SALARY)AS "최대급여" 
...
SUM(SALARY)AS "급여 합",ROUND(AVG(SALARY)) AS "평균급여",
MIN(SALARY)AS "최소급여",MAX(SALARY)AS "최대급여" 
FROM EMPLOYEE GROUP BY JOB_CODE HAVING AVG(SALARY)>=3600000 ORDER BY JOB_CODE;


-- 성별에따른 급여
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여')AS "성별", COUNT(*)AS"성별 수" 
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE ORDER BY DEPT_CODE;
 ...
 FROM ->GROUP BY -> SELECT ->HAVING-> ORDER BY
  ...
  [HAVING 절]
   그룹에 대한 조건을 제시할 때 사용되는 구문(주로 그룹함수식을 가지고 조건을 만듬)
    ...
    WHERE : 모든 행에 관해서 사용한다. (한 행별로 조회)
     
     
    HAVING: 그룹에관한 행에서 사용 (그룹 안에있는 값을 검사)
        GROUP BY절에 사용
        HAVING절은 단일 컬럼 조건이 올수 없고 집계함수에의한 그룹별 조건만이 올수 있다.
...
-- 각 부서별 평균급여가 300만원 이상은 부서들만 조회(부서큳 평균급여)
SELECT DEPT_CODE AS "부서코드",ROUND(AVG(SALARY)) "평균급여"
FROM EMPLOYEE
...
SELECT DEPT_CODE AS "부서코드",ROUND(AVG(SALARY)) "평균급여"
FROM EMPLOYEE
GROUP BY DEPT_CODE HAVING SALARY>=3000000; -- 단일 컬럼으로는 조건을 사용하지 못한다.
...
SELECT DEPT_CODE AS "직급코드" ,COUNT(BONUS) AS "보너스" 
FROM EMPLOYEE
GROUP BY DEPT_CODE 
...
FROM->WHERE->GROUP->HAVING->SELECT->ORDER BY  (순서 매우 중요)

전체 컬럼에서  WHERE 절로 단일 행 조건 처리를 하고 그룹으로 묶어서 조건에 맞게 추출한뒤 선택한후 정렬한다.

데이터 베이스에서 데이터를 잘 쪼개거나 합쳐주어야 한다.
...
-- 부서코드가 D5인 중 급여가 300만원 초과인 사원들을 제외하고 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE ='D5'
MINUS
